...
void RenderTable::paintObject(PaintInfo& paintInfo, const LayoutPoint& paintOffset)
{
    ...
    if (view()->pageLogicalHeight()) {
        ...
        if (m_head) {
            LayoutPoint childPoint = flipForWritingMode(paintOffset);
        ...
        if (m_foot) {
            ...
            if (!info.rect.contains(childPoint.x() + m_foot->x(), childPoint.y() + m_foot->y())) {
                ...
                for (RenderObject* section = firstChild(); section; section = section->nextSibling()) {
                    if (section->isTableSection()) {
                        ...
                        for(RenderObject* row = section->firstChild(); row; row = row->nextSibling()) {
                            ...
                            LayoutPoint rowPoint = flipForWritingMode(paintOffset);
                ...
                repaintedFoot = true;
                repaintedFootPoint = LayoutPoint(childPoint.x(), dy - m_foot->y());
